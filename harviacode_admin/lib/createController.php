...
$string .="\n\n    public function index()
    {   
        ...
        \$data['content_header']          = '".ucfirst($controller)." List';
        \$data['content_header_small']    = '';
        \$data['breadcrumb_active']       = '<li class=active>".ucfirst($controller)."</li>';
...
$string .= "\n\t    );
            //DATA
            \$data['folder_admin']            = \$this->admin_folder;
            \$data['content_header']          = '".ucfirst($controller)."';
            \$data['content_header_small']    = '';
            \$data['breadcrumb_active']       = '<li class=active>".ucfirst($controller)."</li>';
            \$data['show_username']           = \$this->session->userdata('namauser');
            ...
            redirect(site_url('".$folder_admin."$controller'));
        }
    }
    ...
    public function create() 
    {
        \$data = array(
            'button' => 'Create',
            'action' => site_url('".$folder_admin."$controller/create_action'),";
...
$string .= "\n\t);
        //DATA
        \$data['folder_admin']            = \$this->admin_folder;
        \$data['content_header']          = '".ucfirst($controller)." Create';
        \$data['content_header_small']    = '';
        \$data['breadcrumb_active']       = '<li class=active>".ucfirst($controller)."</li>';
        \$data['show_username']           = \$this->session->userdata('namauser');
...
$string .= "\n\t    );
    ...
    public function update(\$id) 
    {
        ...
        if (\$row) {
            \$data = array(
                'button' => 'Update',
                'action' => site_url('".$folder_admin."$controller/update_action'),";
$result2 = mysql_query("SELECT COLUMN_NAME,COLUMN_KEY FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='$database' AND TABLE_NAME='$table'");
if (mysql_num_rows($result2) > 0)
{
    while ($row2 = mysql_fetch_assoc($result2))
...
$string .= "\n\t    );
            //DATA
            \$data['folder_admin']            = \$this->admin_folder;
            \$data['content_header']          = '".ucfirst($controller)." Update';
            \$data['content_header_small']    = '';
            \$data['breadcrumb_active']       = '<li class=active>".ucfirst($controller)."</li>';
            \$data['show_username']           = \$this->session->userdata('namauser');
